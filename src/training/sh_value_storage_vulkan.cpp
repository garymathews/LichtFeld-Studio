    ...
    void ShNMutationBatch::flush() {
        ...
        for (size_t i = 0; i < blocks.size();) {
            ...
            if (start < old_n) {
                ...
                if (splat.shN_value_quantized()) {
                    const auto codes = splat.shN().reshape({-1}).slice(0, start * rest * 3, start * rest * 3 + sh_value_quant::sh_value_u16_count(keep, rest));
                    const auto bounds = splat.shN_value_bounds().reshape({-1, 2}).slice(0, start / 256, (start + keep + 255) / 256);
                    canonical.slice(0, 0, keep).copy_from(sh_value_quant::decode_shN_u16_to_canonical_tensor(codes, bounds, keep, rest));
                } else {
                    const auto values = splat.shN().reshape({-1}).slice(0, sh_swizzled_float_count(start, rest), sh_swizzled_float_count(start + keep, rest));
                    canonical.slice(0, 0, keep).copy_from(undo_reorder_sh_from_swizzled(values, keep, rest, rest).to(DataType::Float32));
                }
            }
