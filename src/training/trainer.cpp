...
namespace lfs::training {
    ...
    void Trainer::submitLossReadback(const lfs::core::Tensor& total_loss, int iter) {
        ...
        slot.host_value = total_loss.cpu().item<float>();
    ...
    std::expected<void, std::string> Trainer::setParams(const lfs::core::param::TrainingParameters& params,
                                                        const std::optional<std::uint64_t> source_serial) {
        const auto reject = [](std::string message) -> std::expected<void, std::string> {
            LOG_ERROR("Rejected training parameter update: {}", message);
            return std::unexpected(std::move(message));
        };
        if (const auto error = params.validate(); !error.empty()) return reject(error);
        ...
        bool running;
        {
            ...
            const auto& old = params_.optimization;
            const auto& next = params.optimization;
            if (initialized_) {
                if (next.strategy != old.strategy || next.sh_degree != old.sh_degree)
                    return reject("Changing training strategy or SH storage degree requires reinitialization");
                if (next.undistort != old.undistort || next.mask_mode != old.mask_mode || next.invert_masks != old.invert_masks ||
                    next.mask_threshold != old.mask_threshold || next.use_alpha_as_mask != old.use_alpha_as_mask ||
                    params.dataset.to_json() != params_.dataset.to_json())
                    return reject("Changing dataset, output, or mask loading settings requires reinitialization");
                if (next.enable_eval && !old.enable_eval && (!val_dataset_ || val_dataset_->size() == 0))
                    return reject("Enabling evaluation requires reinitialization with a validation split");
            }
            if (next.bg_mode == core::param::BackgroundMode::Image) {
                std::error_code ec;
                if (!std::filesystem::is_regular_file(next.bg_image_path, ec))
                    return reject("Background image is not a readable file");
            }
            pending_params_ = params;
            pending_params_source_serial_ = source_serial;
            ++params_revision_;
            parameter_update_result_.reset();
            running = is_running_.load(std::memory_order_acquire);
        }
        if (!running) return apply_pending_params_at_safe_point();
        return {};
    }
    ...
    std::optional<Trainer::ParameterUpdateResult> Trainer::take_parameter_update_result() {
        std::lock_guard lock(params_mutex_);
        return std::exchange(parameter_update_result_, std::nullopt);
    }

    std::expected<void, std::string> Trainer::apply_pending_params_at_safe_point() {
        std::optional<lfs::core::param::TrainingParameters> update;
        uint64_t revision;
        std::optional<std::uint64_t> source_serial;
        {
            std::lock_guard lock(params_mutex_);
            update.swap(pending_params_);
            revision = params_revision_;
            source_serial = pending_params_source_serial_;
        }
        if (!update)
            return {};
        const auto applied = apply_parameter_update(std::move(*update));
        {
            std::lock_guard lock(params_mutex_);
            // A later request supersedes this completion, including its rollback.
            if (revision == params_revision_)
                parameter_update_result_ = ParameterUpdateResult{params_, applied ? std::string{} : applied.error(), source_serial};
        }
        return applied;
    }

    std::expected<void, std::string> Trainer::apply_parameter_update(core::param::TrainingParameters update) {
        const std::unique_lock render_lock(render_mutex_);
        const std::unique_lock model_lock(model_access_mutex_);
        if (const auto error = update.optimization.validate_training_backend(core::default_gpu_backend()); !error.empty()) {
            LOG_ERROR("Rejected pending training parameter update: {}", error);
            return std::unexpected(error);
        }
        const bool reload_image = update.optimization.bg_image_path != params_.optimization.bg_image_path ||
                                  update.optimization.bg_mode != params_.optimization.bg_mode || !bg_image_base_.is_valid();
        if (initialized_ && completed_mutation_epoch_ != mutation_epoch_) {
            LOG_ERROR("Cannot update incomplete training state; restore a checkpoint or reinitialize");
            return std::unexpected("Cannot update incomplete training state; restore a checkpoint or reinitialize");
        }
        core::Tensor image = bg_image_base_;
        core::Tensor color;
        try {
            if (update.optimization.bg_mode != core::param::BackgroundMode::Image) {
                image = {};
            } else if (reload_image) {
                lfs::io::LoadParams load_params{.resize_factor = 1, .max_width = 0, .cuda_stream = nullptr};
                image = lfs::io::CacheLoader::getInstance().load_cached_image(update.optimization.bg_image_path, load_params);
                if (!image.is_valid() || image.ndim() != 3 || image.shape()[0] != 3)
                    throw std::runtime_error("Background image must have three RGB channels");
                image = image.to(core::Device::GPU);
            }
            if (background_.is_valid()) {
                const auto& rgb = update.optimization.bg_color;
                color = core::Tensor::from_vector(std::vector<float>{rgb[0], rgb[1], rgb[2]}, {3}, core::Device::GPU);
            }
        } catch (const std::exception& error) {
            LOG_ERROR("Rejected background update; previous settings retained: {}", error.what());
            return std::unexpected(error.what());
        }
        if (initialized_ && strategy_) {
            waitForModelReaders();
            ++mutation_epoch_;
            auto runtime = update.optimization;
            runtime.iterations = runtime.resolved_total_iterations();
            strategy_->set_optimization_params(runtime);
            strategy_->get_optimizer().set_frozen_lr_scale(update.freeze_lr_scale);
        }
        {
            std::lock_guard lock(params_mutex_);
            std::swap(params_, update);
        }
        if (reload_image || params_.optimization.bg_mode != core::param::BackgroundMode::Image)
            clearBackgroundImageCache();
        bg_image_base_ = std::move(image);
        if (color.is_valid()) background_ = std::move(color);
        if (progress_) progress_->set_total_iterations(get_total_iterations());
        if (evaluator_) evaluator_->set_evaluation_params(params_.optimization);
        if (initialized_ && strategy_) {
            recordParamsReady();
            completed_mutation_epoch_ = mutation_epoch_;
        }
    ...
    void Trainer::handle_control_requests(int iter, std::stop_token stop_token) {
        ...
        if (on_control_poll_)
            on_control_poll_();
        if (const auto applied = apply_pending_params_at_safe_point(); !applied)
            LOG_WARN("Training parameter update rejected: {}", applied.error());
...
#else
        bg_mix_buffer_ = lfs::core::Tensor::from_vector(std::vector<float>(result, result + 3), {3}, lfs::core::Device::GPU);
#endif
    ...
    lfs::Status Trainer::train(std::stop_token stop_token) {
        ...
        try {
            ...
            if (progress_) {
                progress_->start(completed_iteration_.load());
                progress_->update(
                    completed_iteration_.load(),
                    current_loss_.load(),
                    static_cast<int>(strategy_->get_model().size()),
        ...
        try {
            const auto finish_reason = terminal_error ? lfs::io::project::TrainingFinishReason::Error
                : user_stopped ? lfs::io::project::TrainingFinishReason::UserStopped
                               : lfs::io::project::TrainingFinishReason::Completed;
            if (progress_) {
                progress_->complete(finish_reason, terminal_iteration);
            }
            if (evaluator_) {
            ...
            if (progress_ && strategy_) {
                progress_->print_final_summary(static_cast<int>(strategy_->get_model().size()),
                                               terminal_iteration, finish_reason);
            }
            if (PerfBenchCollector::enabled()) {
