...
#include "rendering/rasterizer/vulkan/src/overlay_params.h"
...
namespace lfs::vis {
    ...
    namespace {
        ...
        std::size_t requestedLodPhysicalPages(const std::size_t logical_chunks,
            ...
            // (issue #1295: 94K pages = 16.6 GB on a card with 11.8 free).
            {
                ...
                if (const auto memory = lfs::core::gpu_backend_memory_info(lfs::core::default_gpu_backend());
                    (free_bytes = memory.free_bytes) > 0) {
                ...
                LOG_ERROR("LOD pool sizing: GPU memory budget unavailable; using minimum pool");
        ...
        class RasterizerArenaRenderGuard final {
        public:
            RasterizerArenaRenderGuard() { throw std::runtime_error("CUDA shared raster arena is unavailable on Vulkan"); }
            void noteVulkanRelease(void*, std::uint64_t) const { throw std::logic_error("CUDA arena handoff on Vulkan"); }
        };
        ...
        [[nodiscard]] std::map<std::string, std::string> makeVkSplatSpirvPaths() {
            const std::filesystem::path root = lfs::rendering::vulkan::resolveVkSplatSpirvRoot();
            return {
...
#else
            return std::unexpected("CUDA buffer interop is unavailable; use Vulkan tensor storage");
#endif
}
        ...
        using enum lfs::rendering::vulkan::OverlayParamIndex;
        ...
        [[nodiscard]] lfs::Result<void> copyHostBytesToVulkanTensor(
            ...
            try {
                ...
                std::memcpy(host.data_ptr(), src, byte_count);
                dest.copy_from(host);
        ...
        [[nodiscard]] lfs::Result<void> copyTensorToVulkanTensor(
            ...
            if (!dest.is_valid() || !dest.is_contiguous() || dest.bytes() < byte_count) {
                return lfs::Status::failure(vulkanAuxCopyError(std::format(
                    "VkSplat {} Vulkan dest must be contiguous and have {} bytes available, need {}",
                    label,
                    dest.is_valid() ? dest.bytes() : 0,
                    byte_count)));
            }
            try {
                const size_t element_size = lfs::core::dtype_size(dest.dtype());
                const size_t count = byte_count / element_size;
                if (byte_count % element_size != 0 || count > src.numel()) {
                    return lfs::Status::failure(vulkanAuxCopyError(std::format(
                        "VkSplat {} tensor copy has an unaligned byte count or insufficient source elements", label)));
                }
                if (byte_count == 0)
                    return {};
                const Tensor prepared = src.to(dest.dtype()).contiguous().flatten();
                dest.flatten().slice(0, 0, count).copy_from(prepared.slice(0, 0, count));
            } catch (const std::exception& error) {
                return lfs::Status::failure(lfs::make_error(lfs::ErrorInit{
                    .code = lfs::ErrorCode::InvalidArgument,
                    .domain = lfs::ErrorDomain::Vulkan,
...
#else
        return std::unexpected("Quantized LOD page uploads are unavailable on Vulkan");
#endif
}
...
#else
        return std::unexpected("CUDA shared scratch is unavailable on Vulkan");
#endif
}
...
if (cudaBackendUsable() && !render_stream_) {
    ...
    lfs::Status VksplatViewportRenderer::waitForModelReads() {
        if (!context_ || !render_complete_timeline_ || last_submitted_render_value_ == 0)
            return {};
        VkSemaphoreWaitInfo wait_info{VK_STRUCTURE_TYPE_SEMAPHORE_WAIT_INFO};
        wait_info.semaphoreCount = 1;
        wait_info.pSemaphores = &render_complete_timeline_;
        wait_info.pValues = &last_submitted_render_value_;
        lfs::rendering::WaitContext wait_ctx;
        wait_ctx.fingerprint = "vksplat.native_model_read";
        wait_ctx.owner_quarantine_flag = &gpu_wait_quarantined_;
        auto outcome = lfs::rendering::wait_semaphores_bounded(
            context_->device(), wait_info, {}, {}, wait_ctx);
        if (!outcome)
            return lfs::Status::failure(outcome.error());
        if (*outcome != lfs::rendering::WaitOutcome::Ready) {
            return lfs::Status::failure(lfs::make_error(lfs::ErrorInit{
                .code = lfs::ErrorCode::DeadlineExceeded,
                .domain = lfs::ErrorDomain::Rendering,
                .user_message = std::format("Viewport model read did not complete: {}", formatWaitFailure(outcome)),
                .detection = LFS_SOURCE_SITE_CURRENT()}));
        }
        return {};
    }
...
if (cudaBackendUsable() && !render_stream_) {
...
#else
                    return std::unexpected("CUDA polygon mask clear is unavailable on Vulkan");
#endif
...
#else
                    return std::unexpected("CUDA ring clear is unavailable on Vulkan");
#endif
...
#else
                return std::unexpected("CUDA ring selection is unavailable on Vulkan");
#endif
    ...
    std::expected<VksplatViewportRenderer::RenderResult, std::string> VksplatViewportRenderer::render(
        ...
        const bool higs_candidate =
            !request.gut && renderer_.supportsMacroRaster() && !synchronize_input_upload &&
            !depth_capture_mode_;
