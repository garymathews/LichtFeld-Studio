...
namespace lfs::python {
    ...
    bool PyTensor::is_cuda() const {
        return tensor_.is_gpu() && lfs::core::gpu_backend_of(tensor_) == lfs::core::GpuBackend::CUDA;
    }
    ...
    nb::tuple PyTensor::dlpack_device() const {
        if (lfs::core::gpu_backend_of(tensor_) == lfs::core::GpuBackend::Vulkan)
            throw std::runtime_error("Vulkan tensor DLPack export is unsupported; transfer to CPU first");
    ...
    PyTensor PyTensor::from_dlpack(nb::object obj) {
...
#else
                throw std::runtime_error("CUDA DLPack import is unavailable on Vulkan; transfer the source to CPU first");
#endif
            } else {
                capsule = nb::cast<nb::capsule>(dlpack_fn());
            }
        ...
        if ((dl.device.device_type == kDLCUDA || dl.device.device_type == kDLCUDAManaged) &&
            lfs::core::default_gpu_backend() != lfs::core::GpuBackend::CUDA)
            throw std::runtime_error("CUDA DLPack storage cannot be imported into the Vulkan tensor backend");
    ...
    namespace {
        Device parse_device(const std::string& device) {
            if (device == "vulkan" && lfs::core::default_gpu_backend() == lfs::core::GpuBackend::Vulkan)
                return Device::GPU;
    ...
    void register_tensor(nb::module_& m) {
        nb::class_<PyTensor>(m, "Tensor")
            ...
            .def_prop_ro("backend", &PyTensor::device,
