...
#include "internal/tensor_index_validation.hpp"
#include <algorithm>
#include <cmath>
#include <cstring>
#include <execution>
...
namespace lfs::core {
    ...
    namespace {
        ...
        template <>
        Float16 masked_fill_cast<Float16>(float value) {
    ...
    namespace internal {
        void assert_index_tensor(const Tensor& indices,
            ...
            if (gpu_backend_of(indices) == GpuBackend::Vulkan) {
                // Reduce bounds before issuing any write. Only two integers
                // crosses to the host for valid indices; retain the detailed
                // position/value diagnostic below on the exceptional path.
                const int lower = check_bounds ? (allow_negative ? -static_cast<int>(upper_bound) : 0)
                                               : std::numeric_limits<int>::min();
                const int upper = check_bounds ? static_cast<int>(upper_bound) - 1
                                               : std::numeric_limits<int>::max();
                const auto input = indices.contiguous();
                pin_operands({&input});
                auto bounds = internal::allocate_like(input, TensorShape{2}, DataType::Int64);
                // The Vulkan kernel supports exact Int64 reductions; the public
                // Tensor reduction contract intentionally has a narrower dtype set.
                for (size_t i = 0; i < 2; ++i) {
                    auto output = bounds.slice(0, i, i + 1);
                    const internal::ReduceProgram program{
                        .op = i == 0 ? ReduceOp::Min : ReduceOp::Max,
                        .result_dtype = DataType::Int64};
                    internal::backend_ops_for(input).reduce(
                        internal::storage_ref(input), internal::storage_ref(output),
                        internal::strided_layout(input), program, {});
                }
                const auto range = bounds.cpu();
                if (range.ptr<int64_t>()[0] >= lower && range.ptr<int64_t>()[1] <= upper)
                    return;
            }
    ...
    using internal::assert_index_tensor;
    namespace {
    ...
    Tensor Tensor::masked_select(const Tensor& mask) const {
        ...
        if (device_ == Device::CUDA) {
            ...
            case DataType::Float16:
                masked_select_cpu(ptr<Float16>(), mask.ptr<unsigned char>(), result.ptr<Float16>(), numel());
                break;
            case DataType::Int32:
    ...
    Tensor& Tensor::masked_fill_(const Tensor& mask, float value) {
        ...
        if (device_ == Device::CUDA) {
            ...
            case DataType::Float32:
                ...
                break;
            case DataType::Float16:
                masked_fill_cpu(ptr<Float16>(), mask_data, numel(), stored_value);
                break;
            case DataType::Int32:
...
#if LFS_TENSOR_CUDA
            if (device_fault_assert_path && internal::gpu_backend_tag(*this) == GpuBackend::CUDA) {
    ...
    void MaskedTensorProxy::operator=(const Tensor& other) {
        ...
        if (tensor_->device() == Device::CUDA) {
            ...
            case DataType::Float32:
                ...
                break;
            case DataType::Float16:
                masked_scatter_cpu(const_cast<Tensor*>(tensor_)->ptr<Float16>(), mask,
                                   other.ptr<Float16>(), tensor_->numel());
                break;
            case DataType::Int32:
