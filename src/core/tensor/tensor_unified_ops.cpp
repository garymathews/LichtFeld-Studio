...
namespace lfs::core {
            ...
            if (result.device_ == Device::CUDA) {
                ...
                if (result.dtype_ == DataType::Float32) {
                    ...
                    if (value == 0.0f) {
                        ...
                        std::vector<Float16> temp(result.numel(), static_cast<Float16>(value));
                        upload(temp.data());
                    }
    ...
    Tensor Tensor::multinomial(const Tensor& weights, int num_samples, bool replacement,
        ...
        // Vulkan validates with a GPU reduction; CUDA's sampler needs this host check.
        if (dense_weights.device() == Device::GPU &&
            internal::storage_ref(dense_weights).backend == GpuBackend::CUDA) {
