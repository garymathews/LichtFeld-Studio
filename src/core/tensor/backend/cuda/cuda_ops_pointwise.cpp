...
namespace lfs::core::internal {
    namespace {
        ...
        void launch_unary_same(const PointwiseProgram& program,
            ...
            case DataType::Int32:
                tensor_ops::launch_unary_op_generic(
                    cuda_pointer<const int>(input), cuda_pointer<int>(output),
                    count, op, context.cuda_stream);
                return;
            default:
        ...
        void launch_unary_bool(const PointwiseProgram& program,
            ...
            case DataType::Float32:
                tensor_ops::launch_unary_op_generic(
                    cuda_pointer<const float>(input), cuda_pointer<unsigned char>(output),
                    count, op, context.cuda_stream);
                return;
            case DataType::Int32:
                tensor_ops::launch_unary_op_generic(
                    cuda_pointer<const int>(input), cuda_pointer<unsigned char>(output),
                    count, op, context.cuda_stream);
                return;
            case DataType::UInt8:
            case DataType::Bool:
                tensor_ops::launch_unary_op_generic(
                    cuda_pointer<const unsigned char>(input),
        ...
        void launch_binary_same(const PointwiseProgram& program,
            ...
            case DataType::Float16:
                tensor_ops::launch_binary_op_generic(
                    cuda_pointer<const __half>(lhs), cuda_pointer<const __half>(rhs),
                    cuda_pointer<__half>(output), count, op, context.cuda_stream);
                return;
            case DataType::Int32:
                tensor_ops::launch_binary_op_generic(
                    cuda_pointer<const int>(lhs), cuda_pointer<const int>(rhs),
                    cuda_pointer<int>(output), count, op, context.cuda_stream);
                return;
            case DataType::Int64:
                tensor_ops::launch_binary_op_generic(
                    cuda_pointer<const int64_t>(lhs), cuda_pointer<const int64_t>(rhs),
                    cuda_pointer<int64_t>(output), count, op, context.cuda_stream);
                return;
            case DataType::UInt8:
                tensor_ops::launch_binary_op_generic(
                    cuda_pointer<const uint8_t>(lhs), cuda_pointer<const uint8_t>(rhs),
        ...
        void launch_binary_bool(const PointwiseProgram& program,
            ...
            case DataType::Bool:
                tensor_ops::launch_binary_op_generic(
                    cuda_pointer<const unsigned char>(lhs),
                    cuda_pointer<const unsigned char>(rhs),
                    cuda_pointer<unsigned char>(output), count, op, context.cuda_stream);
                return;
            default:
...
#define LFS_LAUNCH_BROADCAST_SAME(Type, Dtype)                            \
    case DataType::Dtype:                                                 \
        tensor_ops::launch_broadcast_binary(                              \
            cuda_pointer<const Type>(lhs), cuda_pointer<const Type>(rhs), \
...
#undef LFS_LAUNCH_BROADCAST_BOOL
        ...
        void launch_direct_scalar(const PointwiseProgram& program,
            ...
            tensor_ops::launch_scalar_op_generic(
                cuda_pointer<const float>(input), program.scalar.value.float_value,
                cuda_pointer<float>(output), count, op, context.cuda_stream);
        }
        ...
        void launch_conversion(const StorageRef input, const StorageRef output,
            ...
            tensor_ops::launch_convert_type(
                cuda_pointer<const Src>(input), cuda_pointer<Dst>(output),
