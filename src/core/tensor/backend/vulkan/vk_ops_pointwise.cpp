...
namespace lfs::core::internal {
    namespace {
        ...
        void dispatch_pointwise(const PointwiseProgram& program,
            ...
            for (size_t offset = 0; offset < count; offset += chunk_elements) {
                ...
                vk::record_dispatch(*context, pipeline, push, std::span<const StorageRef>(reads.data(), read_count), writes, dispatch_groups(*context, vectorized ? chunk / 4 : chunk),
                                    1, 1);
            }
        }
        ...
        void dispatch_fill(const StorageRef output, const StridedLayout& layout,
            ...
            const std::array writes{output};
            vk::record_dispatch(*context, pipeline, push, {}, writes, dispatch_groups(*context, layout.element_count),
                                1, 1);
        }
    ...
    void VulkanBackendOps::broadcast_binary(
        ...
        const std::array writes{output};
        vk::record_dispatch(*context, pipeline, push, reads, writes, dispatch_groups(*context, output_layout.element_count),
                            1, 1);
    }
    ...
    void VulkanBackendOps::fused_pointwise_chain(
        ...
        for (size_t offset = 0; offset < count; offset += chunk_elements) {
            ...
            vk::record_dispatch(*context, pipeline, push, reads, writes, dispatch_groups(*context, chunk), 1, 1);
        }
    }
