...
namespace lfs::core::internal {
    ...
    void VulkanContext::initialize_runtime() {
        ...
        LOG_INFO("Tensor Vulkan backend initialized: {} (device {}, UUID {}, subgroup {}, "
                 "host upload path=staging-ring, device {})",
    ...
    void VulkanContext::adopt_device(const AdoptedDevice& adopted) {
        ...
        with_other_queues_idle_ = adopted.with_other_queues_idle;
    ...
    VulkanContext::~VulkanContext() {
        try {
            ...
            // The logger may already be destroyed during static teardown.
            std::fputs("Vulkan final teardown could not establish completion; native resources retained until process exit\n", stderr);
        }
    }
    ...
    void VulkanContext::retire_resources(const std::function<void()>& work) {
        try {
            if (with_other_queues_idle_ && !dead())
                with_other_queues_idle_(work);
            else
                work();
        } catch (const lfs::Exception& error) {
            if (error.error().code() == ErrorCode::DeviceLost)
                mark_device_lost_once();
            else if (error.error().code() == ErrorCode::DeadlineExceeded)
                accepting_work_.store(false, std::memory_order_release);
            throw;
        }
    }

    void VulkanContext::require_work() {
        if (dead()) {
            vk_check(this, VK_ERROR_DEVICE_LOST, "admit Vulkan work");
        }
        if (!accepting_work()) {
            throw lfs::Exception(lfs::make_error(lfs::ErrorInit{
                .code = ErrorCode::Unavailable,
                .domain = lfs::ErrorDomain::Vulkan,
                .user_message = "Vulkan backend is not accepting work; restart and restore the last checkpoint.",
                .detection = LFS_SOURCE_SITE_CURRENT(),
            }));
        }
    }
        ...
        try {
            {
                ...
                value = next_timeline_.fetch_add(1, std::memory_order_relaxed) + 1;
            ...
            wait(value);
        } catch (const lfs::Exception& completion) {
            auto error = completion.error();
            if (failure) {
                lfs::core::run_guarded<void>(
                    {.name = "external-vulkan-work", .domain = lfs::ErrorDomain::Vulkan, .operation_id = lfs::OperationId::generate(), .site = LFS_SOURCE_SITE_CURRENT()},
                    [&]() -> lfs::Status { std::rethrow_exception(failure); },
                    [&](lfs::Status&& original) {
                        error = std::move(error).with_suppressed(original.error());
                    });
            }
        ...
        for (uint32_t slice = 0;; ++slice) {
            ...
            if (slice == 59) {
                ...
                throw lfs::Exception(lfs::make_error(lfs::ErrorInit{
                    ...
                    .domain = lfs::ErrorDomain::Vulkan,
                    .user_message = "Vulkan work stalled; restart and restore the last checkpoint.",
                    .detail = std::format("Timeline {} did not complete within 120 s; resources retained", value),
                    .detection = LFS_SOURCE_SITE_CURRENT(),
                }));
            }
        }
    ...
    lfs::Status adopt_vulkan_context(const AdoptedDevice& adopted) {
        ...
        std::call_once(slot->once, [&] {
            try {
                auto context = std::make_shared<VulkanContext>(adopted);
                std::lock_guard lock(g_context_mutex);
    ...
    bool vulkan_backend_probe_available() noexcept {
        if (const auto live = try_live_vulkan_context()) {
            return !live->dead() && live->accepting_work();
        }
    ...
    std::shared_ptr<VulkanContext> acquire_vulkan_context() {
        ...
        LFS_ASSERT_MSG(context != nullptr, "Vulkan backend was shut down during initialization");
        return context;
    }
    ...
    void shutdown_vulkan_context() {
        ...
        std::call_once(slot->once, [] {});
        if (const auto context = slot->context)
            context->shutdown();
        std::lock_guard lock(g_context_mutex);
        if (g_context_slot == slot)
            g_context_slot = std::make_shared<ContextSlot>();
