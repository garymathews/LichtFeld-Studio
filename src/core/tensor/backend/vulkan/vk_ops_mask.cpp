...
namespace lfs::core::internal {
    namespace {
        ...
        struct Compaction {
            vk::ScopedAllocation scan;
            vk::ScopedAllocation total;
        };

        Compaction scan_predicate(VulkanContext& context, const uint32_t predicate,
                                  const StorageRef mask, const size_t count) {
            const uint32_t elements = checked_u32(count, "Vulkan mask count exceeds uint32");
            Compaction result{
                .scan = vk::ScopedAllocation(context, count * sizeof(uint32_t)),
                .total = vk::ScopedAllocation(context, 16),
            };
            const MaskPush push{
                .mask_address = address(mask),
                .source_address = address(result.total.storage()),
                .scan_address = address(result.scan.storage()),
                .count = elements,
            };
            const std::array reads{mask};
            const std::array writes{result.scan.storage(), result.total.storage()};
        ...
        size_t compact_nonzero(const uint32_t predicate, const StorageRef input,
            ...
            const MaskPush push{
                ...
                .scan_address = address(compaction.scan.storage()),
                .count = checked_u32(program.count, "Vulkan nonzero count exceeds uint32"),
            };
            const std::array reads{input, compaction.scan.storage()};
            ...
            const uint32_t total = read_total(*context, compaction.total.storage());
            return total;
        }
    } // namespace
    ...
    size_t VulkanBackendOps::masked_select(
        ...
        const std::array reads{input, mask, compaction.scan.storage()};
        const std::array writes{output};
    ...
    void VulkanBackendOps::masked_scatter(
        ...
        const std::array reads{mask, source, compaction.scan.storage()};
        const std::array writes{output};
    ...
    void VulkanBackendOps::where(
        ...
        const std::array writes{output};
        vk::record_dispatch(*context, pipeline, push, reads, writes, dispatch_groups(*context, output_layout.element_count), 1, 1);
    }
