...
namespace lfs::core::internal {
    namespace {
        ...
        void validate_layout(const StridedLayout& layout) {
            ...
            (void)checked_u32(layout.element_count, "Vulkan strided count exceeds uint32");
        ...
        void dispatch_strided(const StorageRef input, const StorageRef output,
            ...
            StridedPush push{
                ...
                .count = static_cast<uint32_t>(layout.element_count),
            };
            ...
            vk::record_dispatch(*context, pipeline, push, reads, writes, dispatch_groups(*context, layout.element_count),
                                1, 1);
        }
        ...
        void dispatch_cat_input(const StorageRef input, const StorageRef output,
            ...
            const std::array reads{input};
            const std::array writes{output};
            vk::record_dispatch(*context, pipeline, push, reads, writes, dispatch_groups(*context, count), 1, 1);
        }
    } // namespace
    ...
    void VulkanBackendOps::strided_upload(
        ...
        const auto context = acquire_vulkan_context();
        const vk::ScopedAllocation staging_storage(*context, source_bytes);
        StorageRef staging = staging_storage.storage();
    ...
    void VulkanBackendOps::pad(
        ...
        const std::array reads{input};
        const std::array writes{output};
        vk::record_dispatch(*context, pipeline, push, reads, writes, dispatch_groups(*context, input_layout.element_count),
                            1, 1);
    }
