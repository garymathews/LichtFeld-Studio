...
namespace lfs::core::internal {
    namespace {
        ...
        void record_index(VulkanContext& context, const Launch& launch,
            ...
            vk::record_dispatch(context, pipeline, push, reads, writes, dispatch_groups(context, launch.total), 1, 1, push.fault_address != 0);
        }
        ...
        void scatter_sorted(VulkanContext& context, Launch launch, const uint32_t run_op,
            ...
            const vk::RadixSort sorted(context, indices,
                                       SortProgram{.dim_size = launch.push.index_size},
                                       vk::RadixSort::IndexKeys);
    ...
    void VulkanBackendOps::scatter(
        ...
        const auto context = acquire_vulkan_context();
        const Launch launch = scatter_launch(kSortedRunMode, output, indices, source,
