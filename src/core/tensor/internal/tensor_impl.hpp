...
namespace lfs::core {
    ...
    namespace detail {
        ...
        constexpr const char* tensor_cpp_type_name() {
            ...
            if constexpr (std::is_void_v<Value>)
                ...
                return "float";
            else if constexpr (std::is_same_v<Value, Float16>)
                return "Float16";
    ...
    struct LoadArgs {
        ...
        std::variant<
            ...
            std::pair<void*, bool>>
            args;
        std::optional<uint64_t> seed; // Explicit draws do not advance the global generator.
    };
                ...
                if (numel() > 0) {
                    ...
                    if (device_ == Device::CUDA) {
                        ...
                        case DataType::Float32:
                            ...
                            break;
                        case DataType::Float16: {
                            const Float16* left_ptr = ptr<Float16>();
                            const Float16* right_ptr = other.ptr<Float16>();
                            Float16* out_ptr = result.ptr<Float16>();
                            const size_t n = result.numel();
                            for (size_t i = 0; i < n; ++i) {
                                out_ptr[i] = static_cast<Float16>(
                                    op(static_cast<float>(left_ptr[i]), static_cast<float>(right_ptr[i])));
                            }
                            break;
                        }
                        case DataType::Int32:
        ...
        const T* ptr() const {
            ...
            if constexpr (!std::is_void_v<Value>) {
                const bool dtype_matches =
                    ...
                    (std::is_same_v<Value, Float16> && dtype_ == DataType::Float16) ||
...
#undef LFS_DEFINE_SCALAR_CMP_OP
        ...
        T item() const {
            ...
            // Note: Float16 check omitted as it's only available in CUDA compilation units
