            ...
            static Tensor eval(const BinaryExpr<LeftExpr, RightExpr, BinaryOp>& /* expr */,
                ...
                if (left_tensor.dtype() == DataType::Float16 && right_tensor.dtype() == DataType::Float16) {
                    ...
                    if (device == Device::CUDA) {
                        ...
                        if (!needs_broadcast) {
                            pin_operands({&left_tensor, &right_tensor});
                            const Float16* left_ptr = left_tensor.template ptr<Float16>();
                            const Float16* right_ptr = right_tensor.template ptr<Float16>();
                            Float16* out_ptr = result.template ptr<Float16>();
                            size_t n = result.numel();
                            for (size_t i = 0; i < n; ++i) {
                                float l = static_cast<float>(left_ptr[i]);
                                float r = static_cast<float>(right_ptr[i]);
                                out_ptr[i] = static_cast<Float16>(op(l, r));
                            }
                        } else {
                            ...
                            const Float16* left_ptr = left_broadcast.template ptr<Float16>();
                            const Float16* right_ptr = right_broadcast.template ptr<Float16>();
                            Float16* out_ptr = result.template ptr<Float16>();
                            size_t n = result.numel();
                            for (size_t i = 0; i < n; ++i) {
                                float l = static_cast<float>(left_ptr[i]);
