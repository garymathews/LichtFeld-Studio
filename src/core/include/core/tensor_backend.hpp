...
namespace lfs::core {
    ...
    struct VulkanDeviceHandles {
        ...
        // Block submission/encoding on the owner's other queues, drain them,
        // and run retirement before reopening admission. Must not call Tensor APIs.
        std::function<void(const std::function<void()>&)> with_other_queues_idle;
    };
    ...
    // Checks the live context without initializing or submitting GPU work.
    LFS_CORE_API lfs::Status vulkan_backend_status();
    // Propagate a fatal failure detected by another consumer of the shared device.
    LFS_CORE_API void invalidate_vulkan_backend(const lfs::Error& error);
    // Physically destroy addressable buffers after all shared-device users finish.
    // The caller must already have retired its explicit uses of these resources.
    LFS_CORE_API void retire_vulkan_resources(void* device, const std::function<void()>& work);
